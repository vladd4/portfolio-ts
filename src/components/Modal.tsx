...
import { handleSubmit } from "../modalController";

const Modal:FC = () => {
    const [inputValue, setinputValue] = useState<string>('')
    return (
        <div id="modal" className="w-2/6 py-10 px-5 rounded-lg bg-light-bg top-1/2 left-1/2 fixed -translate-x-1/2 -translate-y-1/2 transition-all duration-300 opacity-0 max-[650px]:w-3/6 max-[400px]:w-5/6">
            <form className="grid place-items-center">
                ...
                <input value={inputValue} onChange={(e) => {setinputValue(e.target.value)}} className="text-[calc(calc(1vh+1vw)/1.4)] border w-full mb-2 py-1.5 px-2 rounded-md outline-none" type="email" placeholder="example@gmail.com"></input>
